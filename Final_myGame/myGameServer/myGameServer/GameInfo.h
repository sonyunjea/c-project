...
private:
    ...
    // 떨어져 있는 구역을 연결하는 함수
    void link(int index, int a, int b) {

        for (int i = 0; i < occupatedArea[index][a].size(); i++) {
            bool flag = true;
            for (int j = 0; j < occupatedArea[index][b].size(); j++) {

                // 가는 길에 다른 구역이 없는지 검토
                int x, y;
                x = occupatedArea[index][a][i][1];
                y = occupatedArea[index][a][i][0];

                flag = true;
                while (x > occupatedArea[index][b][j][1]) {
                    x--;
                    if (map[y][x] == MAPSTATE::PLACE_P1 + (index + 1) % 2) {
                        flag = false; break;
                    }
                }
                while (x < occupatedArea[index][b][j][1]) {
                ...
                while (y > occupatedArea[index][b][j][0]) {
                    y--;
                    if (map[y][x] == MAPSTATE::PLACE_P1 + (index + 1) % 2) {
                        flag = false; break;
                    }
                ...
                if (flag) {
                    ...
                    while (x > occupatedArea[index][b][j][1]) {
                        x--;
                        map[y][x] = MAPSTATE::PLACE_P1 + index;
                    }
                    while (x < occupatedArea[index][b][j][1]) {
                        x++;
                        map[y][x] = MAPSTATE::PLACE_P1 + index;
        ...
        if (key == 32) {
                ...
                // 점령 상태
                    ...
                    if (players[index].isAbleOccupation(index, map, (index == 0)?MAPSTATE::PLACE_P2:MAPSTATE::PLACE_P1)) {
                        ...
                        if (occupatedArea[index].size() > 1) {
                            // 가장 최근과 그 전의 구역 연결
                            link(index, occupatedArea[index].size() - 2, occupatedArea[index].size() - 1);
                        }
